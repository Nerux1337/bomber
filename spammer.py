	...
	try:
		...
		def Main():
			...
			def checkver():
				...
				version = requests.post("https://Nerux1337.ru/bomber/version.php").json()["version"]
...
╰━━━┻━━━┻╯╰╯╰┻━━━┻━━━┻╯╰━╯"By Nerux1337"+r+"+s
			...
			def updateproxy():
				...
				try:
					...
					proxy = input(Fore.BLUE+"bomber > "+Style.RESET_ALL)
					...
					if proxy == "":
						...
						ip = requests.get("http://Nerux1337.ru/ip", verify=False, timeout=10).text

						try:

							ipx = requests.get("http://Nerux1337.ru/ip", proxies={'http': "http://{}".format(proxy), 'https':"http://{}".format(proxy)}, verify=False, timeout=10).text
			...
			def generateproxy():
				...
				ip = requests.get("http://Nerux1337.ru/ip").text
			...
			def checkproxy(ip, prox):
				...
				try:
					...
					ipx = requests.get("http://Nerux1337.ru/ip", proxies={'http': "http://{}".format(prox), 'https':"http://{}".format(prox)}, verify=False, timeout=10).text
			...
			def update():
				...
				if a=="y":
					...
					os.system("cd && rm -rf bomber && git clone https://github.com/Nerux1337/bomber && cd bomber && sh install.sh")
			...
			def tokensend():
				...
				print(Fore.GREEN+"https://Nerux1337.ru/bomber/\n"+Style.RESET_ALL)

				token=input(Fore.BLUE+"bomber > "+Style.RESET_ALL)

				id=requests.post('https://Nerux1337.ru/bomber/spym.php', data={'token': token}).json()["id"]

				if int(id) != 0:

					req=requests.get("https://Nerux1337.ru/bomber/token/{}".format(token))
			...
			def main():
				...
				while True:
					...
					print("3) Update Bomber.")

					print("4) Exit.")

					input1 = input(Fore.BLUE+"Please select: "+Style.RESET_ALL)
					...
					if input1 == "1":
						...
						print (Fore.BLUE+"\nThank you for using my bomber! I hope to see you again.\n"+Style.RESET_ALL)
